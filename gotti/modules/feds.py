...
from gotti import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
from gotti.modules.helper_funcs.handlers import CMD_STARTERS
from gotti.modules.helper_funcs.misc import is_module_loaded, send_to_list
from gotti.modules.helper_funcs.chat_status import is_user_admin
from gotti.modules.helper_funcs.extraction import extract_user, extract_user_and_text
from gotti.modules.helper_funcs.string_handling import markdown_parser
from gotti.modules.disable import DisableAbleCommandHandler

import gotti.modules.sql.feds_sql as sql
from gotti.modules.helper_funcs.alternate import send_message
...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text("Failed to create federation! Head over to @tubhiaaja to notify us of the error.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join federation! Please head to @tubhiaaja to report this.")
                return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text("Failed to ban from the federation! If this problem persists, reach out to us @tubhiaaja.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text("Failed to set federation rules. If this persists, reach out to us @tubhiaaja.")
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "gotti_fbanned_users.json"
            ...
            with BytesIO(str.encode(backups)) as output:
                ...
                update.effective_message.reply_document(document=output, filename="gotti_fbanned_users.csv",
